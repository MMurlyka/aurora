...
div
	label.Group-Field
		...
		field-input.Group-Field__Field(name="id" :value="id" disabled)
	label.Group-Field
	...
	.Group-Field
		form-button(@click="update()") Изменить
</template>
...
export default {
	...
	data() {
		return {
			name: this.$store.state.items[this.id].name,
			button: this.$store.state.items[this.id].button,
			piece: this.$store.state.items[this.id].piece,
			discounted: this.$store.state.items[this.id].discounted,
			available: this.$store.state.items[this.id].available,
		};
	},
	// computed: {
	// 	name() {
